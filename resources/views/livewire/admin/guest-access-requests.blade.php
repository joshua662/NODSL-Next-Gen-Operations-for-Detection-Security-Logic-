<div class="space-y-6 max-w-6xl mx-auto px-4">
    ...
    <div class="space-y-4">
        @forelse ($requests as $request)
            <div class="bg-white dark:bg-zinc-800 border border-purple-200 dark:border-purple-700 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
                <!-- Header -->
                <div class="flex justify-between items-start mb-5 pb-4 border-b border-zinc-200 dark:border-zinc-700">
                    <div class="flex-1">
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            <h3 class="text-lg font-semibold text-purple-900 dark:text-purple-100">{{ $request->requested_changes['guest_name'] ?? 'Guest' }}</h3>
                        </div>
                    ...
                    <span class="px-4 py-2 text-sm font-medium rounded-full whitespace-nowrap ml-4
                ...
                <div class="mb-5 pb-4 border-b border-zinc-200 dark:border-zinc-700">
                    ...
                    <div class="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4">
                        <div class="grid md:grid-cols-2 gap-4">
                            ...
                            <div class="border border-purple-200 dark:border-purple-600 rounded p-3">
                                <p class="text-xs font-semibold text-purple-600 dark:text-purple-400 uppercase mb-1">Access Date</p>
                ...
                <div class="flex justify-between items-center">
                    <div class="text-xs text-zinc-500">
                        @if($request->reviewed_by)
                            Reviewed by <strong>{{ $request->reviewer->name }}</strong> on {{ $request->reviewed_at->format('M d, Y') }}
                        @endif
                    </div>

                    @if($request->status === 'pending')
                        <div class="flex gap-2">
                            <flux:button wire:click="approve({{ $request->id }})" variant="primary" size="sm" class="px-4">✓ Approve</flux:button>
                            <flux:button wire:click="reject({{ $request->id }})" variant="ghost" size="sm" class="text-red-600 px-4">✕ Reject</flux:button>
                        </div>
