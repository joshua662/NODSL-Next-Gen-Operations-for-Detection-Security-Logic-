<div class="space-y-6 max-w-6xl mx-auto px-4">
    ...
    <div class="space-y-4">
        @forelse ($requests as $request)
            <div class="bg-white dark:bg-zinc-800 border border-blue-200 dark:border-blue-700 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
                <!-- Header -->
                <div class="flex justify-between items-start mb-5 pb-4 border-b border-zinc-200 dark:border-zinc-700">
                    <div class="flex-1">
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            <h3 class="text-lg font-semibold text-zinc-900 dark:text-zinc-100">{{ $request->resident->name }}</h3>
                        </div>
                            ...
                            @foreach($request->requested_changes as $key => $value)
                                ...
                                @elseif($key !== 'is_personal_data_request' && $key !== 'request_type')
                                    <li class="text-sm flex items-center justify-between">
                                        <strong class="text-zinc-900 dark:text-zinc-100">{{ ucfirst(str_replace('_', ' ', $key)) }}</strong>
                                        <span class="text-zinc-600 dark:text-zinc-400 font-mono">{{ $value }}</span>
                                    </li>
                                @endif
                            @endforeach
                        </ul>
